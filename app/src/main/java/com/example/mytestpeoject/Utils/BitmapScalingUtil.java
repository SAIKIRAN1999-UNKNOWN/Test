...
//		dbo.inSampleSize
		...
		if(rotatedHeight > 2560)
			rotatedHeight = 2560;
		if(rotatedWidth > 1920)
			rotatedWidth = 1920;
	...
	private static Bitmap readScaledBitmapFromUri(Uri photoUri,
			...
			throws FileNotFoundException, IOException {
		Utils.logI("BitmapScalingUtil Read Scaled Bitmap: " + width + " "
				+ height);
		InputStream is;
		Bitmap srcBitmap;
		...
		if (width > MAX_IMAGE_DIMENSION || height > MAX_IMAGE_DIMENSION) {
			...
			Utils.logI("BitmapScalingUtil Scaled Bitmap: " + ratio);
			...
			Utils.logI("BitmapScalingUtil NOT Scaled Bitmap ");
	...
	private static Bitmap readRoughScaledBitmap(InputStream is, float maxRatio) {
		...
		if (result != null) {
			Utils.logI("BitmapScalingUtil Read Scaled Bitmap Result wtf: "
					+ result.getWidth() + " " + result.getHeight());
			Utils.logI("BitmapScalingUtil MaxRatio wtf: " + maxRatio);
		}
		return result;
	}
	...
	private static int getOrientationFromExif(Uri imageUri, Context context) {
		int orientation = -1;
		Utils.logI("BitmapScalingUtil imageUri = " + imageUri);
		// File imageFile = new File(getRealPathFromUri(imageUri, context));
		File imageFile = new File(imageUri.getPath());
		try {
			ExifInterface exif;
			Utils.logI("BitmapScalingUtil imageFile.getAbsolutePath() = "
